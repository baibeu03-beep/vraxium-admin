...
// 공용 hover/focus 툴팁 프리미티브(네이티브 title 대체).
...
export function Tooltip({
  ...
  const [coords, setCoords] = React.useState<{ top: number; left: number; placement: Placement } | null>(
    null,
  );
  ...
  const show = open && content !== null && content !== undefined && content !== "";
      ...
      {show &&
        ...
        createPortal(
          <div
            ...
            className={cn(
              "max-w-xs whitespace-normal break-words rounded-md bg-foreground px-2.5 py-1.5",
              "text-xs leading-snug text-background shadow-lg ring-1 ring-foreground/10",
              contentClassName,
            )}
          >
            {content}
