...
| 46 | `2026-07-23_experience_overall_review_reset.sql` | 실무 경험 [개설 검수] 취소 — `cluster4_experience_team_overall.status` CHECK 에 `'none'`(검수 전) 추가 + `cluster4_experience_opening_logs.action` CHECK 에 `'review_cancel'`(검수 취소) 추가. 검수 후 파트장이 개설 신청 데이터를 **실제로 바꾸면** 확인 팝업 → 저장 + 검수 취소. **미적용 시에도 코드가 `reviewed_at=NULL` sentinel 로 폴백 동작**(판독 SoT=`lib/experienceReviewResetPolicy.resolveOverallStatus` 가 두 표현을 모두 'none' 으로 읽음)하고, 적용 후에야 정식 `status='none'` + 검수 취소 로그가 남는다. 헤더를 지우지 않으므로 팀장 관리/확장 셀·아웃풋은 보존. `apply-experience-overall-review-reset.ts`(exec_sql 없으면 수동 안내). **제약 확장만·데이터 무접촉** |
...
| 48 | `2026-07-31_process_irregular_acts_hub_line_grade.sql` | 변동 액트(`process_irregular_acts`)에 `hub_grade`(club\|info\|experience\|competency, CHECK) + `line_grade`(항상 'variable_act', CHECK) 컬럼 추가. 기존 행 전부 `hub_grade='club'`·`line_grade='variable_act'` 로 일괄 backfill 후 NOT NULL 적용. 조회 인덱스 `(organization_slug, week_id, scope_mode, hub_grade)` 추가. **nullable 추가 → backfill → NULL 잔여 0 검증(실패 시 전체 ROLLBACK) → NOT NULL/CHECK 순서로 단일 트랜잭션.** 미적용 시에도 코드가 컬럼 존재 여부를 probe 해 select/insert 에서 자동 제외(hub 급 4허브 매칭은 과거처럼 실무 정보 귀속으로 폴백). **적용 완료.** |
| 49 | `2026-08-03_process_irregular_acts_team_scope.sql` | 변동 액트 `hub_grade='experience'` 전용 `team_id`(uuid, `cluster4_teams` FK) + `team_name`(denorm) + `part_scope`(항상 'team_overall') 컬럼 추가. 조사 시점 유일한 experience 행 1건을 `resolvePositionAtBatch`로 실측 backfill. 조회 인덱스 `(organization_slug, week_id, scope_mode, hub_grade, team_id)` 추가. CHECK는 당시 "experience+팀 전부 NULL"도 유예 허용(과거 미배정 행 대비) — #50 에서 강화됨. **적용 완료.** |
| 50 | `2026-08-03_process_irregular_acts_team_scope_strict.sql` | #49 의 유예 CHECK 제거 — experience 는 이제 `team_id`/`team_name` NOT NULL + `part_scope='team_overall'` 필수(양쪽 NULL 유예 삭제). 적용 전 위반 행이 있으면 즉시 EXCEPTION 으로 전체 ROLLBACK. **#49 의존.** |
