...
// PATCH /api/admin/line-opening-windows/[id]  body: { is_active: boolean }
//   예외 활성/비활성 토글. 비활성 = 판정에서 제외(행 보존).
export async function PATCH(request: NextRequest, { params }: Ctx) {
  ...
  try {
    await setLineOpeningWindowActive(id, isActive);
    ...
    console.error("[admin/line-opening-windows/:id PATCH]", error);
    return Response.json(
      {
        success: false,
...
// DELETE /api/admin/line-opening-windows/[id]
//   예외 영구 삭제. 삭제 즉시 해당 주차/라인 개설은 자동 정책만 따른다(차단 복귀).
export async function DELETE(_request: NextRequest, { params }: Ctx) {
  ...
  const { id } = await params;
  try {
    await deleteLineOpeningWindow(id);
    ...
    console.error("[admin/line-opening-windows/:id DELETE]", error);
    return Response.json(
      {
        success: false,
