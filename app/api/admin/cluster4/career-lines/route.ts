...
function parseBody(
  ...
  // output_images — optional. string[] 또는 [{url, caption?}] 둘 다 허용 (info-lines 와 동일).
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 라인 개설 대상자 등록 → Point A·B 즉시 지급(source='line', pay-once). 공통 SoT. best-effort(개설 응답 무영향).
    try {
      await payLineOpenTargetsOnce(lineRow.id);
    } catch (payoutErr) {
      console.warn("[career-lines POST] line payout failed", payoutErr);
    }
