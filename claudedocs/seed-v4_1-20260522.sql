-- ============================================================
-- Seed SQL v4.1: 30명 더미 사용자 (Cluster1 only, no auth/admin)
-- batch_id = '2026-05-22_seed_30users_v1'
-- ============================================================
-- 작성일: 2026-05-22
-- 상태: ACTIVE — public.users 기반 Cluster1 minimum viable seed
--
-- ★ v4 → v4.1 변경:
--   (1) applicants.reviewed_at 컬럼 부재 → approved_at 으로 교체
--       실제 컬럼: id, email, name, provider, status, linked_user_id,
--                  raw_profile, created_at, updated_at, approved_at,
--                  approved_by, rejected_at, admin_note
--   (2) admin_users INSERT 제거 — admin_users.id FK → auth.users(id)
--       → auth.users 와 함께 v5 로 분리
--   (3) 검증 게이트 11중 → 10중 (admin_users=2 조건 제거)
--
-- ★ 기존 더미 식별 (사용자 보고 2026-05-22):
--   encre 4명 / oranke 3명 = 모두 기존 더미 (실사용자 아님)
--   조직 배정 A'안 유지: oranke 신규 20 + encre 신규 10 + phalanx 신규 0
--
-- ★ FK 루트 정리 (사용자 보고 2026-05-22):
--   public.users(id) ← {user_profiles, user_memberships, user_cumulative_points,
--                       user_growth_stats, user_educations}.user_id
--   user_profiles(user_id) ← {applicants.linked_user_id, user_cluster2,
--                             user_introductions, portfolio_*, weekly_*,
--                             user_season_histories, test_user_markers}.user_id
--   auth.users(id) ← admin_users.id     ← v5 에서 처리
--
-- ★ 적용 범위 (v4.1):
--   ✅ public.users
--   ✅ user_profiles
...
-- 0-B: v4 잔여물 4종 zero 확인
SELECT 'auth_users_residue' AS check_name, COUNT(*) AS residue_count
FROM auth.users WHERE lower(email) LIKE 'dummy%@vraxium.test'
UNION ALL
SELECT 'public_users_residue', COUNT(*)
FROM public.users WHERE legacy_user_id BETWEEN 900001 AND 900030
UNION ALL
SELECT 'profiles_residue', COUNT(*)
FROM public.user_profiles WHERE display_name LIKE '[TEST] %'
UNION ALL
SELECT 'markers_residue',
  CASE
...
-- ============================================================
-- ★ Phase 1: test_user_markers 테이블 idempotent CREATE
-- ============================================================
BEGIN;
...
BEGIN
  IF EXISTS (
    ...
    IF old_row_count > 0 THEN
      RAISE EXCEPTION 'test_user_markers row=% 존재. 수동 검토 필요.', old_row_count;
    END IF;
    ...
    IF NOT has_legacy_user_id THEN
      RAISE NOTICE '구버전 test_user_markers — DROP 진행';
      DROP TABLE public.test_user_markers CASCADE;
    ELSE
      RAISE NOTICE 'test_user_markers v3+ 구조 (row=0) 활용';
    END IF;
  END IF;
END $$;
...
-- ============================================================
-- ★ Phase 2-10: 단일 트랜잭션 — 모든 row INSERT + 검증
-- ============================================================
BEGIN;

-- 2: Seed 데이터 TEMP TABLE
...
SELECT
  ...
  CASE
    ...
    ELSE 'status_issue'
  END AS user_type,
  -- 신규 더미 조직 분배: oranke 20 / encre 10 / phalanx 0
...
-- 8: applicants (v4.1 정정 — reviewed_at 제거, approved_at 사용)
...
-- 10: 10중 검증 게이트 (admin_users=2 제거)
DO $$
DECLARE
  users_count int;
  profile_count int;
...
BEGIN
  ...
  IF users_count        <> 30 THEN RAISE EXCEPTION 'users=% (30)',        users_count;        END IF;
  IF profile_count      <> 30 THEN RAISE EXCEPTION 'profiles=% (30)',     profile_count;      END IF;
  IF marker_count       <> 30 THEN RAISE EXCEPTION 'markers=% (30)',      marker_count;       END IF;
  IF oranke_count       <> 20 THEN RAISE EXCEPTION 'oranke=% (20)',       oranke_count;       END IF;
  IF encre_count        <> 10 THEN RAISE EXCEPTION 'encre=% (10)',        encre_count;        END IF;
  IF phalanx_dummy_count <> 0 THEN
    RAISE EXCEPTION 'phalanx_dummy=% (0 — 실사용자 격리 위배)', phalanx_dummy_count;
  END IF;
  IF membership_count   <> 30 THEN RAISE EXCEPTION 'memberships=% (30)',  membership_count;   END IF;
  IF points_count       <> 30 THEN RAISE EXCEPTION 'points=% (30)',       points_count;       END IF;
  IF growth_count       <> 30 THEN RAISE EXCEPTION 'growth=% (30)',       growth_count;       END IF;
  IF applicant_count    <> 30 THEN RAISE EXCEPTION 'applicants=% (30)',   applicant_count;    END IF;

  RAISE NOTICE 'v4.1 검증 통과 (10/10): users=%, profiles=%, markers=%, oranke=%, encre=%, phalanx_dummy=%, memberships=%, points=%, growth=%, applicants=%',
    users_count, profile_count, marker_count, oranke_count, encre_count, phalanx_dummy_count,
    membership_count, points_count, growth_count, applicant_count;
END $$;
...
-- ============================================================
-- Rollback SQL v4.1 (별도 실행 — 4중 AND 마커 검증)
-- ============================================================
/*
BEGIN;
...
BEGIN
  ...
  SELECT COUNT(*) INTO remaining
  FROM public.user_profiles WHERE display_name LIKE '[TEST] %';
  IF remaining > 0 THEN RAISE EXCEPTION '[TEST] 잔존 % 건', remaining; END IF;

  RAISE NOTICE 'Rollback v4.1 완료';
END $$;
...
--     Expected: 34
--
-- (3) phalanx updated_at 변화 (영향 zero):
--     SELECT COUNT(*) FROM public.user_profiles up
--     LEFT JOIN public.test_user_markers tm ON tm.user_id = up.user_id
...
--            COUNT(*) FILTER (WHERE tm.user_id IS NOT NULL) AS new_dummy_count,
--            COUNT(*) FILTER (WHERE tm.user_id IS NULL) AS pre_existing_count
--     FROM public.user_profiles up
--     LEFT JOIN public.test_user_markers tm ON tm.user_id = up.user_id
--     GROUP BY up.organization_slug ORDER BY up.organization_slug;
--     Expected (사용자 보고 기존 더미 포함):
--       encre   | 10 |  4   (10 신규 + 4 기존 더미)
--       oranke  | 20 |  3   (20 신규 + 3 기존 더미)
--       phalanx |  0 | 34   (0 신규 + 34 실사용자)


-- ============================================================
-- 다음 단계 (v5+)
-- ============================================================
-- v5  : auth.users + admin_users + login flow
--       - admin_users.id FK → auth.users(id) 동기 생성
--       - 비밀번호 'TestSeed!2026' (또는 Supabase Auth admin API 사용)
--       - 운영진 2명 (legacy 900027='owner', 900028='admin')
