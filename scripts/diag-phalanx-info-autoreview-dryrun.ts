/**
 * diag-encre-info-autoreview-dryrun.ts — phalanx 실무정보(info) 자동 검수 DRY-RUN (READ-ONLY)
 *
 * 목적: target 0명 + output_link 보유 encre info 라인의 카페 댓글 작성자를 일괄 집계하고
 *       phalanx user_profiles/user_memberships 와 매칭 결과를 보고한다.
...
const CACHE_PATH =
  "C:/Users/vanua/AppData/Local/Temp/claude/C--Users-vanua-OneDrive-Desktop-vraxium-admin/3abe1568-cd41-4d68-84c7-bb9f38a71655/scratchpad/cache-phalanx-info-crawl.json";
...
function vMatchOne(parsed: VParsed, crews: CrewRecord[]): VSingle {
  ...
  if (parsed.format === "new") {
    // phalanx 닉네임은 {이름} {학교} {전공}(token[1]=학교) 형태가 다수다.
    //   encre 매칭기는 token[1]=팀으로만 봐서 학교형을 못 잡으므로, mid(token[1])를
    //   school/team/part 어느 하나와, tail(token[2])을 major 와 대조한다.
    //   자격(qualified) = 이름 일치 AND (mid 가 school/team/part 중 하나 일치 OR major 일치).
    const mid = parsed.teamName; // token[1] — 학교 또는 팀
    const tail = parsed.majorName; // token[2] — 전공
    const nameMatches = crews.filter((c) => veq(c.name, parsed.name));
    const qualified = nameMatches.filter(
      (c) =>
        veq(c.schoolName, mid) ||
        veq(c.teamName, mid) ||
        veq(c.partName, mid) ||
        veq(c.majorName, tail),
    );
    if (qualified.length === 1) {
      const c = qualified[0];
      const field = veq(c.schoolName, mid)
        ? "school"
        : veq(c.teamName, mid)
          ? "team"
          : veq(c.partName, mid)
            ? "part"
            : "major";
      return { status: "auto", crew: c, reason: `new:name+${field}` };
    }
    // 이름이 유일(동명이인 아님) → 안전하게 확정(코호트 폴백과 동일 정책).
    if (nameMatches.length === 1) {
      return { status: "auto", crew: nameMatches[0], reason: "new:name-only" };
    }
    const reason =
      nameMatches.length === 0
        ? "신형: 이름 후보 0명"
        : qualified.length >= 2
          ? `신형: 이름+학교/팀/파트/전공 후보 ${qualified.length}명`
          : `신형: 이름 후보 ${nameMatches.length}명(학교/팀/파트/전공 불일치)`;
    return { status: "review", reason, nameCandidates: nameMatches };
  }
...
async function main() {
  console.log("=== phalanx 실무정보(info) 자동 검수 DRY-RUN (READ-ONLY) ===");
  ...
  const encreInfo = allInfo.filter((l) => parseLineCodeOrg(l.line_code) === "phalanx");
  console.log(`[1] active info 라인 ${allInfo.length}건 중 phalanx(PX 토큰) ${encreInfo.length}건`);
  ...
  const allEncreCrews = await loadCrewRecords("phalanx");
  const scope = await resolveUserScope("operating", "phalanx");
  const crews: CrewRecord[] = scope.filter(allEncreCrews, (c) => c.userId);
  console.log(
    `\n[3] phalanx 크루 레코드 ${allEncreCrews.length}명 로드 → operating(테스트 ${scope.testUserIds.size}명 제외) = ${crews.length}명`,
  );
  ...
  const fullReportPath = "claudedocs/phalanx-info-autoreview-dryrun-full.json";
  ...
  try {
    writeFileSync(
      ...
      JSON.stringify(
        {
          generatedFor: "phalanx info auto-review dry-run (FULL)",
