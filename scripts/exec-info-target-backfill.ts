/**
 * exec-info-target-backfill.ts — encre info 라인 cluster4_line_targets 백필 EXECUTE + 검증
 *
...
const ROLLBACK_OUT = "claudedocs/encre-info-target-backfill-rollback.json";
...
async function main() {
  ...
  for (const l of json.lines) {
    ...
    for (const uid of Array.from(new Set(l.variant.matchedUserIds))) {
      if (userOrg.get(uid) !== "encre") continue;
  ...
  // 비영향 표본(encre 비대상 사용자 10명) before 카드 직렬화 — 불변 검증용.
  const { data: encreUsers } = await sb.from("user_profiles").select("user_id").eq("organization_slug", "encre").order("user_id").limit(400);
  const nonAffected = ((encreUsers ?? []) as any[]).map((r) => r.user_id).filter((u) => !affectedUsers.has(u)).slice(0, 10);
  ...
  const rec = await recomputeWeeklyCardsSnapshotsForUsers(affectedUserIds, { concurrency: 4 });
  ...
  console.log(`[비영향 불변] encre 비대상 표본 ${nonAffected.length}명 snapshot 동일 = ${unchanged}/${nonAffected.length}`);
  ...
  writeFileSync("claudedocs/encre-info-backfill-sample-users.json", JSON.stringify({ sample, affectedCount: affectedUserIds.length }, null, 2));
}
