...
// 프로덕션과 동일한 라벨 presenter — 픽스처가 status ↔ label 을 어긋나게 만들지 않도록 여기서도 쓴다.
...
function makeRow(p: Partial<CrewWeekLineDetailRow> & { partType: Cluster4LinePartType }): CrewWeekLineDetailRow {
  // ⚠ enhancementLabel 은 **status 에서 파생**한다 — 프로덕션(adminCrewWeekLineSummary)이
  //   enhancementLabel: formatEnhancementStatusLabel(enhancementStatus) 로 둘을 항상 함께 세팅하므로,
  //   상수 기본값을 쓰면 status=fail 인데 라벨="강화 성공"인 **실재 불가능한 행**이 만들어져
  //   라벨 검증이 통째로 무의미해진다(실제로 그랬다).
  const status: Cluster4EnhancementStatus = p.enhancementStatus ?? "success";
  return {
    ...
    enhancementLabel: p.enhancementLabel ?? formatEnhancementStatusLabel(status),
...
console.log("\n[케이스 1] 클럽 오픈 8 — admin 판정 그대로(성공 4 / 실패 4). 비배정 fail 도 강화 실패.");
{
  const rows = [
    ...
    ...Array.from({ length: 3 }, (_, i) =>
      // 클럽 오픈 + 비배정 + admin=fail → 크루도 **강화 실패**. 배정 여부로 재분류하지 않는다.
  ...
  check("crewOpenCount", dto.summary.crewOpenCount, 8);
  check("successCount", dto.summary.successCount, 4);
  check("failureCount (배정 1 + 비배정 3)", dto.summary.failureCount, 4);
  check("notApplicableCount (admin 이 na 로 판정한 행 없음)", dto.summary.notApplicableCount, 0);
  check("enhancementRate (4/8)", dto.summary.enhancementRate, 50);
  assertInvariants(dto, "케이스1");
}

console.log("\n[케이스 2] 전부 해당 없음 — admin 이 not_applicable 로 판정한 행만");
{
  // ⚠ 해당 없음의 유일한 근거 = admin enhancementStatus === "not_applicable".
  //   (비배정+fail 은 케이스 1 에서 강화 실패로 검증된다 — 여기 섞으면 분모 0 의도가 깨진다.)
  const rows = Array.from({ length: 3 }, (_, i) =>
    unassigned({ partType: "experience", lineId: `n${i}`, enhancementStatus: "not_applicable" }),
  );
  const dto = project(rows);
...
console.log("\n[케이스 2-b] 회귀 — admin=강화 실패(ltid=null, 미제출) → 크루도 강화 실패");
{
  // 2026-07-17 실측 회귀(정보 "인포데스크"): clubOpen + lineTargetId=null + submissionStatus=not_submitted
  //   + admin enhancementStatus=fail(reason=target_missing_required) 인 행이 크루에서 "해당 없음"으로
  //   재분류되던 버그. 배정/제출/포인트/평점 어느 것도 결과를 덮지 않아야 한다.
  const rows = [
    unassigned({
    ...
    // 미제출이지만 정책상 성공(정보/경험 마감 후 성공 처리) — 제출 여부로 결과를 덮지 않는다.
    assigned({
      partType: "information",
      lineId: "wisdom",
  ...
  const dto = project(rows);
  check(
    "ltid=null + admin fail → 강화 실패",
    [dto.rows[0].result, dto.rows[0].resultLabel, dto.rows[0].resultTone],
    ["failure", "강화 실패", "danger"],
  );
  check(
    "미제출 + admin success → 강화 성공",
    [dto.rows[1].result, dto.rows[1].resultLabel],
    ["success", "강화 성공"],
  );
  check("포인트 0 이어도 해당 없음으로 바뀌지 않음", dto.summary.notApplicableCount, 0);
  check("평점 null 이어도 해당 없음으로 바뀌지 않음", dto.rows[0].rating, null);
  assertInvariants(dto, "케이스2-b");
}
...
console.log("\n[케이스 4] 포인트 합계 — 행별 earned/available 합 === summary");
{
  const rows = [
    ...
    // 비배정(획득 0) 행도 클럽 오픈이면 가능치가 있다 → "0 / N" (성공 행만 합산 금지).
  ...
  check("비배정 실패 행 pointA (0 / N)", dto.rows[2].pointA, { earned: 0, available: 5 });
...
console.log("\n[추가] 관리자 전용 필드 비노출 + 결과 라벨/톤");
{
  const rows = [
    ...
    unassigned({ partType: "experience", lineId: "c", enhancementStatus: "not_applicable" }),
  ...
  check("admin not_applicable → 해당 없음", [dto.rows[2].result, dto.rows[2].resultLabel, dto.rows[2].resultTone], ["not_applicable", "해당 없음", "neutral"]);
