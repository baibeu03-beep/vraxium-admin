/**
 * Pilot v2 — 5명 PMS 이관 apply (preview 기본 · --apply 시에만 write · --rollback <runlog>).
 *
 *   npx tsx --env-file=.env.local scripts/apply-pms-pilot-5.ts            # preview (write 0)
 *   npx tsx --env-file=.env.local scripts/apply-pms-pilot-5.ts --apply
 *   npx tsx --env-file=.env.local scripts/apply-pms-pilot-5.ts --rollback <runlog.json>
 *
 * 대상 (2026-06-07 승인): P1 oranke 1092 장승완 · P2 hrdb 1463 안은비 · P3 olympus 249 성채윤
 *   · P4 olympus 248 박시은 · P5 olympus 251 정혜빈.
 *
 ...
 *
 * fail-closed 게이트 (하나라도 걸리면 apply 차단):
 *   - EXPECTED(승인 기대값)와 preview 집계 불일치 (drift) — ensure 라인 distinct 수 포함
 *   - 페어 점유·테스터 매칭·cross-source 기록·모호 매칭
 *
...
const OUT = `claudedocs/pilot-apply-5-${MODE}-${STAMP}.json`;
const CREATED_BY = "pilot-apply-5";
const DEFAULT_THRESHOLD = 30;
const RATING_FAIL_MAX = 3;
const UNIFIED_MASTER_NAME = "[통합] 주차 활동 내역";

const PILOT: Array<{ p: string; src: PmsSourceSystem; uid: number; name: string }> = [
...
// 승인 기대값 (pilot-apply-5-plan-20260607.md v2 + 라인 ensure 정책) — drift 시 apply 차단.
const EXPECTED = {
  usersInsert: 3,
  usersUpdate: 2,
  profilesInsert: 3,
  ...
  uwpOverwrite: 8,
  sentinel: 5,
  checksMigratedFalse: 2,
  ledger: 3043,
  expEach: 137,
  snapshotNew: 3,
  snapshotRecompute: 2,
  ensureLinesDistinct: 11, // [통합] 라인 ensure 생성 예정 distinct 주차 (조사 2026-06-07)
};
...
async function rollback(file: string) {
  ...
  for (const l of log.insertedLines ?? []) {
    const { error } = await sb
      ...
      .delete()
      .eq("id", l.id)
      .eq("source_file_name", "pilot-apply-5");
...
async function main() {
  ...
  const drift: string[] = [];
  const cmp = (k: keyof typeof EXPECTED, actual: number, note = "") => {
    if (actual !== EXPECTED[k]) drift.push(`${k}: 기대 ${EXPECTED[k]} ≠ 실측 ${actual}${note}`);
  };
  cmp("usersInsert", agg.usersInsert);
  cmp("usersUpdate", agg.usersUpdate);
  cmp("profilesInsert", agg.profilesInsert);
  cmp("uwsInsert", agg.uwsInsert);
  cmp("uwsOverwrite", agg.uwsOverwrite);
  cmp("uwpInsert", agg.uwpInsert);
  cmp("uwpOverwrite", agg.uwpOverwrite);
  cmp("sentinel", agg.sentinel);
  cmp("checksMigratedFalse", agg.checksMigratedFalse);
  cmp("ledger", agg.ledger - plans.length, " (adjustment 5행 제외 기준)");
  cmp("expEach", agg.expEach);
  cmp("snapshotNew", agg.snapshotNew);
  cmp("snapshotRecompute", agg.snapshotRecompute);
  cmp("ensureLinesDistinct", agg.ensureLinesDistinct); // ensure 예정 수가 승인값(11)과 다르면 drift
  const blocked = RESUME ? false : drift.length > 0;
  if (RESUME && drift.length) console.log(`(--resume) drift ${drift.length}건 기록만 — 부분 적용 후 재개 상태:`, drift.join(" | "));
  ...
  const report = {
    generatedAt: `2026-06-07 pilot-apply-5 ${MODE}`,
    mode: MODE, blocked, drift, totals: agg, expected: EXPECTED, perUser,
