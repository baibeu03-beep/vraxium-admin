/**
 * diag-info-target-backfill-dryrun.ts — phalanx info 라인 cluster4_line_targets 백필 DRY-RUN
 *
 * 입력: claudedocs/phalanx-info-autoreview-dryrun-full.json (이미 생성된 전수 크롤 산출물)
...
const PLAN_OUT = "claudedocs/phalanx-info-target-backfill-plan.json";
...
async function main() {
  console.log("=== phalanx info cluster4_line_targets 백필 DRY-RUN (write 0) ===\n");
  ...
  // ── 3) 매칭 user_id 가 user_profiles(phalanx) 에 실존하는지 검증 ─────────────────
  ...
  for (const l of jsonLines) {
    ...
    for (const uid of matched) {
      ...
      if (validUser.get(uid) !== "phalanx") { nonPhalanxUserSkip++; continue; }
  ...
  console.log(`기타 skip — 비활성 ${inactiveSkip} · 비info ${nonInfoSkip} · week_id 없음 ${nullWeekSkip} · user 미존재 ${missingUserSkip} · 비phalanx ${nonPhalanxUserSkip}`);
  if (Object.keys(skippedDetail).length) console.log(`  라인 미존재 skip:`, skippedDetail);
  console.log(`정합: JSON pair ${jsonPairCount} = insert ${plan.length} + dedup ${dedupSkip} + skip ${inactiveSkip + nonInfoSkip + nullWeekSkip + missingUserSkip + nonPhalanxUserSkip}`);
    ...
    JSON.stringify(
      {
        ...
        linesAffected,
        dedupSkip,
        otherSkip: { inactiveSkip, nonInfoSkip, nullWeekSkip, missingUserSkip, nonPhalanxUserSkip },
