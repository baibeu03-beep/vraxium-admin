...
// 설명(description)·이미지 설명(imageDescription)도 필수값이므로 기본값을 채운다 —
//   링크/이미지 우선순위만 보는 케이스가 설명 누락으로 먼저 걸리지 않도록.
//   설명 2종의 필수 검증은 verify-experience-output-description-required-http.ts 가 전담한다.
function outputs(
  link: string,
  imageUrl: string,
  description = "설명",
