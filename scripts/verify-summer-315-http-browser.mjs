// 검증 — /admin/members 모집단 315: HTTP roster == direct + 브라우저(전체315·시즌휴식50·중단66).
//   사전조건: admin :3000. node scripts/verify-summer-315-http-browser.mjs
...
ck("HTTP 모집단 = 315 (633 아님)", members.length === 315, `${members.length}`);
const byId = new Map(members.map((m) => [m.userId, m.displayGrowthStatus]));
ck("휴식50·중단66·active199 코호트 전원 명부 포함", [...rest, ...stopped, ...active].every((id) => byId.has(id)) && (rest.size + stopped.size + active.size) === 315);
const dist = {};
...
ck("direct 코호트(199/50/66=315) == HTTP 모집단", members.length === 315 && active.size === 199 && rest.size === 50 && stopped.size === 66);
...
ck("브라우저 모집단 315", rosterCount === 315, `${rosterCount}`);
...
const r1 = await applyFilterAndCount("시즌 휴식"); console.log(`  시즌 휴식 필터=${r1}`); ck("브라우저 시즌 휴식 50", r1 === 50, `${r1}`);
const r2 = await applyFilterAndCount("활동 중단"); console.log(`  활동 중단 필터=${r2}`); ck("브라우저 활동 중단 66", r2 === 66, `${r2}`);
await page.screenshot({ path: "claudedocs/members-315.png" });

await browser.close();
hr();
console.log(fail === 0 ? "✅ /admin/members 315 모집단 HTTP+BROWSER PASS" : `❌ ${fail} FAILED`);
