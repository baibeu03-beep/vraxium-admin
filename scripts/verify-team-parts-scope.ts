...
async function main(){
  ...
  ck("[direct operating] (T) 테스트팀 0 노출", oNames.filter(n=>isTestTeam(ORG,n)).length===0, `view=${oNames.length}`);
  ...
  ck("[HTTP operating] (T) 0 노출", hO.filter(n=>isTestTeam(ORG,n)).length===0, `view=${hO.length}`);
