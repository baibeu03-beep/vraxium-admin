...
async function main(){
  ...
  // QA 실사용자 숨김(QA_HIDE_REAL_USERS): QA 중엔 operating 도 test 축 → 운영 팀 누수 0(빈 목록 허용:
  //   팀 반편성에 (T) 팀 미등록 가능). QA 종료 후 = (T) 팀 누수 0. 양쪽 다 "축 밖 팀 0 노출"로 검증.
  const opOk = (ns:string[]) => QA_HIDE_REAL_USERS
  ...
  ck(`[direct operating] ${QA_HIDE_REAL_USERS?"운영 팀 0 노출(QA)":"(T) 테스트팀 0 노출"}`, opOk(oNames), `view=${oNames.length}`);
  ...
  ck(`[HTTP operating] ${QA_HIDE_REAL_USERS?"운영 팀 0 노출(QA)":"(T) 0 노출"}`, opOk(hO), `view=${hO.length}`);
