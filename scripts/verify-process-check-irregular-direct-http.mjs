// 비정규 액트 (/admin/processes/check/irregular) — direct(DB) == HTTP 검증.
...
try {
  ...
  const guard = await api("/api/admin/processes/check/irregular", { method: "POST", body: J({
    organization: ORG, kind: "manual_grant", act_name: `${TAG} 가드`, target_user_ids: [teTarget.user_id],
  }) });
  ...
  const guardOrg = await api("/api/admin/processes/check/irregular", { method: "POST", body: J({
    organization: ORG, kind: "manual_grant", act_name: `${TAG} 가드org`, target_user_ids: [encreOp.user_id],
  }) });
  ...
  // ── 14. 고객앱 무영향 — 비정규 액트가 user_weekly_points 에 행 생성 안 함 ──
  ...
  ck("[고객앱 무영향] user_weekly_points 는 비정규 액트로 변하지 않음(read-only 확인)", !uwpRows.error || uwpRows.error.code !== "PGRST205", `uwp count=${uwpRows.count ?? "n/a"}`);
