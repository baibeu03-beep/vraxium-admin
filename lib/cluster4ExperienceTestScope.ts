...
//
// ⚠ QA 실사용자 숨김 정합(lib/qaFixedScope.QA_HIDE_REAL_USERS): resolveUserScope 는 QA 기간에 전달
//   mode 와 무관하게 test 모집단(테스트 유저)으로 고정한다. 팀 목록(=선택창)도 같은 population 축을
//   써야 "팀 드롭다운 ↔ 크루/파트" 모집단이 일치한다(운영 팀에 테스트 크루 0명 → 파트/크루 빈칸 방지).
//   팀 목록은 "사람을 고르는 선택창"이므로 이 population 스위치의 대상이다(정책/주차 축과 무관).
//
